<template>
  <div class="fixed bottom-4 sm:bottom-8 left-1/2 -translate-x-1/2 z-[100] w-[92%] max-w-lg pointer-events-none lg:ml-72">
    <nav class="pointer-events-auto bg-white/95 backdrop-blur-2xl rounded-[2rem] p-2 flex items-center justify-around shadow-xl border border-slate-100 relative">
      <router-link 
        ...
        :to="item.path"
        v-slot="{ isActive }"
        class="relative px-3 py-2 group flex-1"
      >
...
<script setup>
const navItems = [
  { name: 'Fleet Hub', path: '/rider/dashboard', icon: 'fas fa-shield-halved' },
  { name: 'Active Mission', path: '/rider/task', icon: 'fas fa-truck-fast' },
  { name: 'Financials', path: '/rider/earnings', icon: 'fas fa-vault' },
  { name: 'Identity', path: '/rider/profile', icon: 'fas fa-user-gear' }
];
</script>
